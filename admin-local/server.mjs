...
function readFluidHomeInfo() {
  ...
  return { blog_title: blogTitle, slogan, about_name: aboutName, about_intro: aboutIntro };
}

function writeFluidHomeInfo({ blog_title = '', slogan = '', about_name = '', about_intro = '' }) {
...
function readBannerInfo() {
  const s = fs.readFileSync(FLUID_CONFIG_FILE, 'utf8');
  const banner = (s.match(/banner_img:\s*(.*)$/m) || [])[1] || '';
  return { banner_img: banner.trim() };
}

function writeBannerInfo({ banner_img = '' }) {
  let s = fs.readFileSync(FLUID_CONFIG_FILE, 'utf8');
  s = s.replace(/banner_img:\s*.*$/m, `banner_img: ${banner_img}`);
  fs.writeFileSync(FLUID_CONFIG_FILE, s, 'utf8');
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'DELETE' && url.pathname.startsWith('/api/posts/')) {
    ...
    if (!fs.existsSync(p)) return send(res, 404, { message: '文件不存在' });
    fs.unlinkSync(p);
    return send(res, 200, { message: '已删除：' + file });
  }

  if (req.method === 'POST' && url.pathname === '/api/posts') {
    const data = JSON.parse((await readBody(req)) || '{}');
    if (!data.title) return send(res, 400, { message: '缺少标题' });
    const file = data.file ? path.join(POSTS_DIR, data.file) : path.join(POSTS_DIR, `${slugify(data.title)}.md`);
    fs.writeFileSync(file, buildPostContent(data), 'utf8');
  ...
  if (req.method === 'GET' && url.pathname === '/api/site-config') {
    return send(res, 200, readSiteConfig());
  }

  if (req.method === 'POST' && url.pathname === '/api/site-config') {
    const data = JSON.parse((await readBody(req)) || '{}');
    writeSiteConfig(data);
    return send(res, 200, { message: '站点配置已保存' });
  }

  if (req.method === 'GET' && url.pathname === '/api/home-info') {
    return send(res, 200, readFluidHomeInfo());
  }

  if (req.method === 'POST' && url.pathname === '/api/home-info') {
    const data = JSON.parse((await readBody(req)) || '{}');
    writeFluidHomeInfo(data);
    return send(res, 200, { message: '首页信息已保存' });
  }

  if (req.method === 'GET' && url.pathname === '/api/banner') {
    return send(res, 200, readBannerInfo());
  }

  if (req.method === 'POST' && url.pathname === '/api/banner') {
    const data = JSON.parse((await readBody(req)) || '{}');
    writeBannerInfo(data);
    return send(res, 200, { message: 'Banner 已保存' });
  }

  if (req.method === 'POST' && url.pathname === '/api/upload-image') {
    const raw = await readBody(req);
    const part = parseMultipart(req, raw);
    if (!part) return send(res, 400, { message: '上传失败' });
    fs.mkdirSync(UPLOAD_DIR, { recursive: true });
    const ext = path.extname(part.filename || '').toLowerCase() || '.png';
    const name = `${Date.now()}${ext}`;
    const filePath = path.join(UPLOAD_DIR, name);
    fs.writeFileSync(filePath, Buffer.from(part.content, 'binary'));
    const publicPath = `/images/uploads/${name}`;
    return send(res, 200, { message: '上传成功', markdown: `![](${publicPath})` });
  }

  if (req.method === 'POST' && url.pathname === '/api/preview') {
    return send(res, 200, await ensureHexoServer());
  }
  ...
  if (req.method === 'POST' && url.pathname === '/api/action') {
    ...
    if (data.action === 'git-push') {
      const a = await run('git add . && git commit -m "update from local admin"');
